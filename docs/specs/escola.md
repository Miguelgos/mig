...
Não há API oficial disponível — a integração usa Puppeteer para navegar pelo portal e Gemini Vision para extrair as informações da tela via OCR.
...
4. Tira screenshot da área de comunicados
        │
        ▼
5. Gemini Vision descreve o estado atual da página (para debug)
        │
        ▼
6. Gemini Vision extrai os comunicados visíveis como JSON estruturado
        │
        ▼
7. Para cada comunicado: clica pelo título, screenshot da tela aberta,
   Gemini Vision extrai o conteúdo completo em `detalhes` e volta
        │
        ▼
...
## Uso do Gemini Vision

Duas chamadas são feitas ao Gemini com o screenshot:

1. **Descrição de estado (debug)**: prompt pedindo ao Gemini para descrever o que está visível na tela. Útil para diagnosticar falhas de login ou carregamento.

2. **Extração de comunicados**: prompt estruturado pedindo ao Gemini para retornar um array JSON com os comunicados visíveis, respeitando a interface `Comunicado`.
...
| Horário | Ação |
|---------|------|
| Diário 7h | Busca comunicados, filtra importantes com Gemini, envia e-mail (com `.ics` se houver data) |
| Diário 13h | Idem |
| Diário 18h | Idem |
...
- **Lento**: cada consulta leva ~15-30 segundos (login + carregamento + Gemini Vision)
